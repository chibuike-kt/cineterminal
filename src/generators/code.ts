...
import { pick, randInt, randHex, randIPv4, chance } from '../utils/random';
...
const RUST_LINES = [
  ...
  `    connect("${randIPv4()}:443").await`,
  `}`,
];

type Lang = 'python' | 'bash' | 'go' | 'c' | 'rust';

const LANG_POOLS: Record<Lang, string[]> = {
  python: PY_LINES,
  bash:   BASH_LINES,
  go:     GO_LINES,
  c:      C_LINES,
...
let currentLang: Lang = 'bash';
let currentIndex = 0;

export function generateCodeLine(): string {
  const pool  = LANG_POOLS[currentLang];
  const color = LANG_COLORS[currentLang];

  if (currentIndex >= pool.length) {
    // Switch language
    const langs = Object.keys(LANG_POOLS) as Lang[];
    currentLang  = pick(langs);
    currentIndex = 0;
    return chalk.dim(''); // blank line between blocks
  }

  const raw  = pool[currentIndex++];
  // Fill any template vars
  const line = raw
    .replace(/\${randIPv4\(\)}/g, randIPv4())
    .replace(/\${randHex\(\)}/g,  randHex(4));

  // Syntax-style coloring based on content
  if (raw.startsWith('#!') || raw.startsWith('#!/')) return chalk.hex('#666666')(line);
  if (raw.startsWith('#'))   return chalk.hex('#555555')(line);
  if (raw.startsWith('//'))  return chalk.hex('#555555')(line);
  if (raw.startsWith('import') || raw.startsWith('use ') || raw.startsWith('#include')) {
    return chalk.hex('#c586c0')(line);
  }
  if (raw.includes('func ') || raw.includes('async def') || raw.includes('fn ') || raw.includes('void ')) {
    return chalk.hex('#dcdcaa')(line);
  }
  if (raw.includes('printf') || raw.includes('println') || raw.includes('print(') || raw.includes('echo ')) {
    return chalk.hex('#ce9178')(line);
  }
  if (raw.trim().startsWith('//') || raw.trim().startsWith('*')) {
    return chalk.hex('#6a9955')(line);
  }
