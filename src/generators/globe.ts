...
import { randFloat, randInt, pick } from '../utils/random';

const LAND_ZONES: Array<[number, number, number, number]> = [
  [25,  72,  -168, -52 ],
  [-56, 15,  -82,  -34 ],
  [34,  72,  -12,  45  ],
  [-36, 38,  -20,  55  ],
...
export interface GlobeState {
  ...
  pingPoints: Array<{ lat: number; lon: number; age: number }>;
}

export function createGlobeState(): GlobeState {
  return {
    rotation: 0,
    targetLat: randFloat(-40, 55),
    targetLon: randFloat(-120, 120),
    lockProgress: 0,
    phase: 'rotating',
    pingPoints: [],
  };
...
export function renderGlobe(state: GlobeState, width: number, height: number): string[] {
  ...
  for (let row = 0; row < height; row++) {
    ...
    for (let col = 0; col < width; col++) {
      ...
      if (dist2 > 1.0) {
        ...
        if (Math.random() < 0.004) line += chalk.hex('#111111')('.');
        else line += ' ';
        continue;
      }
      ...
      const distToTarget = Math.sqrt(
        Math.pow(lat - state.targetLat, 2) +
        Math.pow(lon - state.targetLon, 2)
      );

      // Ping points
      const nearPing = state.pingPoints.find(p =>
        Math.sqrt(Math.pow(lat - p.lat, 2) + Math.pow(lon - p.lon, 2)) < 10
      );
      ...
      if (isLocked) {
        line += chalk.hex('#ff0000').bold('?');
      } else if (onCrossH && onCrossV) {
        line += chalk.hex('#ff6600').bold('+');
      } else if (onCrossH && onTarget) {
        line += chalk.hex('#ff4400')('-');
      } else if (onCrossV && onTarget) {
        line += chalk.hex('#ff4400')('¦');
      } else if (onCrossH) {
        line += chalk.hex('#662200')('-');
      } else if (onCrossV) {
        line += chalk.hex('#662200')('¦');
      } else if (nearPing) {
        line += nearPing.age < 5
          ? chalk.hex('#00ff41').bold('?')
          : chalk.hex('#007722')('?');
      } else if (edgeGlow) {
        // Limb of the sphere — bright edge
        line += chalk.hex('#004422').bold('¦');
      } else if (isLand(lat, lon)) {
        // Land with diffuse shading
        if      (diffuse > 0.75) line += chalk.hex('#00aa44').bold('¦');
        else if (diffuse > 0.55) line += chalk.hex('#007733')('¦');
        else if (diffuse > 0.35) line += chalk.hex('#005522')('¦');
        else                      line += chalk.hex('#002211')('¦');
      } else {
        // Ocean with grid lines and shading
        const gridLat = Math.abs(lat % 30) < 2;
        const gridLon = Math.abs(lon % 30) < 3.5;
        if (gridLat && gridLon) {
          line += chalk.hex('#003322')('+');
        } else if (gridLat || gridLon) {
          line += chalk.hex('#001a11')('·');
        } else if (diffuse > 0.65) {
          line += chalk.hex('#001e14')('¦');
        } else if (diffuse > 0.35) {
          line += chalk.hex('#00120d')('·');
        } else {
          line += chalk.hex('#000a07')('·');
        }
      }
    }
    rows.push(line);
...
export function generateCoordLine(): string {
  const city   = pick(CITIES);
  const lat    = (city.lat  + randFloat(-0.5, 0.5)).toFixed(4);
  const lon    = (city.lon  + randFloat(-0.5, 0.5)).toFixed(4);
  const alt    = randInt(200, 35000);
  const sig    = randInt(60, 99);
  const status = pick([
    chalk.hex('#00ff41')('ACQUIRED'),
    chalk.hex('#ffcc00')('SCANNING'),
    chalk.hex('#ff4444')('BLOCKED '),
    chalk.hex('#00aaff')('RELAYING'),
  ]);
  return (
    chalk.hex('#00cc44')(`${city.name.padEnd(12)} `) +
    chalk.dim(`${lat}°  ${lon}°  `) +
    chalk.hex('#005522')(`ALT:${String(alt).padStart(5)}m `) +
    chalk.dim(`SIG:${sig}% `) +
    status
  );
}

export function generateSatLine(): string {
  const sats = [
    'SAT-7A','RELAY-03','NEXUS-1','GHOST-9',
    'PHANTOM','UPLINK-4','DARKSTAR','ORACLE-6',
  ];
  const sat    = pick(sats);
  const el     = randInt(5, 90);
  const az     = randInt(0, 359);
  const snr    = randFloat(12, 45).toFixed(1);
  const status = pick([
    chalk.hex('#00ff41').bold('? ONLINE '),
    chalk.hex('#ffcc00')('? HANDSHK'),
    chalk.hex('#ff4444')('? OFFLINE'),
    chalk.hex('#00aaff')('? UPLINK '),
  ]);
  return (
    status + '  ' +
    chalk.hex('#00cc33')(sat.padEnd(10)) +
    chalk.dim(`EL:${String(el).padStart(2)}°  AZ:${String(az).padStart(3)}°  `) +
    chalk.hex('#004422')(`SNR:${snr}dB`)
  );
}
